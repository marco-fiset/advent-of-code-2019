...
(defn move-to-coord [move]
  (let [[dir & dist] move
        distance (read-string (clojure.string/join dist))]
    (case dir
      \U [0 (- distance)]
      \D [0 distance]
      \L [(- distance) 0]
...
(defn path-to-coords [path]
  (:coords (reduce (fn [res move]
                     (let [coord (move-to-coord move)
                           new-point (map + (:point res) coord)]
                       (-> res
                           (update :coords conj new-point)
                           (assoc :point new-point))))
                   {:point [0 0] :coords []}
                   path)))

(defn between? [p1 p2 p3]
  (let [start (min p2 p3)
        end (max p2 p3)]
    (and (< start p1)
         (< p1 end))))

(defn intersect? [segment1 segment2]
  (let [[[x1 y1] [x2 y2]] segment1
        [[x3 y3] [x4 y4]] segment2]
      ;; if segment1 is horizontal and segment2 intersects it or vice versa
      ;; and segment1 is vertical and segment2 intersects it or vice versa
    (and (or (and (= x1 x2)
                  (between? y3 y1 y2))
             (and (= x3 x4)
                  (between? y1 y3 x4)))
